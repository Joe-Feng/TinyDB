 ...
 *
 * LeafFlag：标记该节点是否为叶子节点  1位
 * KeyNumber：该节点中 key 的个数   2位
 * SiblingUid：其兄弟节点存储在 DM 中的 UID    8位
 * [Son0][Key0][Son1][Key1]...[SonN][KeyN]：
 * 穿插的子节点（SonN）和 KeyN。最后的一个 KeyN 始终为 MAX_VALUE，以此方便查找。

 *
 * 当节点内部错误发生时，即当 Ti 在对节点的数据进行更改时，TinyDB 发生了崩溃。
 * 由于 IM 依赖于 DM，在数据库重启后，
 * Ti 会被撤销（undo），对节点的错误影响会被消除。
 */
public class Node {
